...
class Report extends Component
{
    ...
    public function mount(){
        ...
        // $this->list['location']=$this->all->unique('Location')->pluck('Location') ;
        /* dd($this->list['location']);
        foreach ($this->all->unique('Location')->pluck('Location') as $rec) {
            array_push($this->list['location'],$rec->Location);
        } */
        // Location
    ...
    {
        $answer = Answer::whereIn('questions_id',[2])
        ...
        if($answer->count()){
            ...
            foreach ($this->data as $key => $value) {
                $locationPieModel[$key]=(new PieChartModel())
                                ->setTitle($key)
                                ->addSlice('score_1',$value['score_1'],'#d8402b')
                                ->addSlice('score_2',$value['score_2'],'#fd740d')
                                ->addSlice('score_3',$value['score_3'],'#ffd302')
                                ->addSlice('score_4',$value['score_4'],'#a1d22a')
                                ->addSlice('score_5',$value['score_5'],'#25b83f');
            }
        }else{
            $this->data=null;
        }
        ...
        if($this->pagination['location']){
            $raw->where('location','like','%'.$this->pagination['location'].'%');
        }
        
        if(is_numeric($this->pagination['show'])){
            $raw=$raw->paginate($this->pagination['show']);
        }else{
            $raw=$raw->get();
        }
